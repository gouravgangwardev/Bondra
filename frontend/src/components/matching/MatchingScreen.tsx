...
const MatchingScreen: React.FC<MatchingScreenProps> = ({
  ...
  if (state === 'idle' || state === 'selecting') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[600px] p-8 bg-bg-primary rounded-2xl">
        
        {/* Header */}
        <div className="text-center mb-8">
          <div className="w-20 h-20 mx-auto mb-4 rounded-3xl bg-primary flex items-center justify-center shadow-xl shadow-primary/20">
            <svg className="w-10 h-10 text-text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
          </div>
          <h2 className="text-2xl font-bold text-text-primary mb-2">Meet Someone New</h2>
          <p className="text-sm text-text-secondary">Choose how you want to chat</p>
        </div>
              ...
              <button
                ...
                className={`
                  ...
                  ${isSelected
                    ...
                    : 'bg-bg-secondary/80 border-border-subtle hover:border-border-default/80 hover:bg-bg-surface/90'
                  }
                `}
              >
                ...
                `}>
                  <div className={`w-6 h-6 ${isSelected ? 'text-white' : 'text-text-secondary'}`}>
                    {mode.icon}
                  </div>
                </div>

                <h3 className={`font-semibold text-sm mb-1 ${isSelected ? 'text-white' : 'text-text-primary'}`}>
                  {mode.label}
                </h3>
                <p className={`text-xs ${isSelected ? 'text-white/70' : 'text-text-disabled'}`}>
                  {mode.description}
                </p>
        ...
        <button
          ...
          className={`
            ...
            ${selectedMode
              ? 'bg-primary hover:bg-primary-hover text-white shadow-primary/25 active:scale-95'
              : 'bg-bg-surface/90 text-text-disabled cursor-not-allowed'
            }
          `}
        >
  ...
  if (state === 'searching') {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[600px] p-8 bg-bg-primary rounded-2xl relative overflow-hidden">
          ...
          {[0, 1, 2].map(i => (
            <div
              ...
              className={`absolute w-64 h-64 rounded-full border-2 ${
                selectedConfig ? `border-${selectedConfig.gradient.split('-')[1]}-500/20` : 'border-primary/20'
              }`}
              style={{
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="relative mb-8">
            <div className={`
              ...
              bg-gradient-to-br ${selectedConfig?.gradient || 'from-primary to-primary-hover'}
              shadow-2xl
            `}>
          ...
          <h2 className="text-2xl font-bold text-text-primary mb-2">Finding a match...</h2>
          <p className="text-sm text-text-secondary mb-6">
            {selectedConfig?.label} · {formatWaitTime(waitTime)} elapsed
          </p>

          {/* Progress bar */}
          <div className="w-64 h-2 bg-bg-surface/90 rounded-full overflow-hidden mb-8">
            <div
              className={`h-full bg-gradient-to-r ${selectedConfig?.gradient || 'from-primary to-primary-hover'}`}
              style={{
          ...
          {/* Tips */}
          <div className="bg-bg-secondary/80 border border-border-subtle rounded-xl p-4 max-w-md mb-6">
            <p className="text-xs text-text-secondary text-center leading-relaxed">
              💡 <strong className="text-text-secondary">Tip:</strong> Be friendly and respectful. You can skip or report users who violate our community guidelines.
            </p>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold text-text-secondary hover:text-text-primary bg-bg-secondary/80 hover:bg-bg-surface/90 border border-border-subtle hover:border-border-default transition-all"
          >
  ...
  if (state === 'matched' && matchedUser) {
    const selectedConfig = modeOptions.find(m => m.type === selectedMode);
    const gradient = selectedConfig?.gradient || 'from-primary to-accent';
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[600px] p-8 bg-bg-primary rounded-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h2 className="text-3xl font-bold text-text-primary mb-2">Match Found! 🎉</h2>
          <p className="text-sm text-text-secondary mb-8">Get ready to chat</p>

          {/* Matched user */}
          <div className="flex items-center gap-4 mb-8 p-5 bg-bg-secondary/80 border border-border-subtle rounded-2xl">
            ...
            <div>
              <p className="text-base font-semibold text-text-primary">{matchedUser.username}</p>
              <p className="text-xs text-text-secondary mt-0.5">{selectedConfig?.label}</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            {onSkipMatch && (
              <button
                onClick={onSkipMatch}
                className="flex items-center gap-2 px-6 py-4 rounded-2xl font-semibold text-sm text-text-secondary hover:text-text-primary bg-bg-secondary/80 hover:bg-bg-surface/90 border border-border-subtle hover:border-border-default transition-all"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
  ...
  if (state === 'failed') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[600px] p-8 bg-bg-primary rounded-2xl">
        ...
        <h2 className="text-xl font-bold text-text-primary mb-2">No Match Found</h2>
        ...
        <button
          onClick={onStartSearch}
          className="flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-sm bg-primary/15 text-primary border border-primary/25 hover:bg-primary-hover/25 transition-all"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
