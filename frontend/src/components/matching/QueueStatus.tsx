...
const QueueStatus: React.FC<QueueStatusProps> = ({
  ...
  if (compact) {
    ...
    return (
      <div className="flex items-center gap-4 px-4 py-3 rounded-xl bg-bg-secondary/80 border border-border-subtle">
        {stats.map(stat => {
          ...
          return (
            ...
            >
              <div className={`w-7 h-7 rounded-lg ${config.iconBg} flex items-center justify-center text-text-primary shrink-0`}>
          ...
          <div
            ...
            className={`
              ...
              ${isCurrent
                ...
                : 'bg-bg-secondary/80 border-border-subtle hover:border-border-default'
              }
            `}
          >
            {/* Background pattern */}
            ...
            {isCurrent && (
              <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/10 backdrop-blur-sm border border-white/20">
                ...
                <span className="text-[10px] font-bold text-text-primary uppercase tracking-wider">Active</span>
              </div>
            )}
            ...
            <div className="relative p-5">
              ...
              <div className={`
                ...
                ${isCurrent ? 'shadow-lg' : ''}
              `}>
                <div className={`w-6 h-6 ${isCurrent ? 'text-text-primary' : 'text-text-secondary'}`}>
                  {config.icon}
                </div>
              </div>

              {/* Label */}
              <h3 className={`text-sm font-bold mb-3 ${isCurrent ? 'text-text-primary' : 'text-text-primary'}`}>
                {config.label}
              </h3>
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex items-center justify-between">
                  <span className={`text-xs ${isCurrent ? 'text-text-primary/70' : 'text-text-secondary'}`}>
                    In Queue
                  </span>
                  <span className={`text-sm font-bold tabular-nums ${isCurrent ? 'text-text-primary' : 'text-text-secondary'}`}>
                    {stat.usersInQueue}
                  </span>
                </div>
              ...
              {/* Progress bar */}
              <div className={`mt-4 h-1 rounded-full overflow-hidden ${isCurrent ? 'bg-white/20' : 'bg-bg-surface/90'}`}>
                <div
